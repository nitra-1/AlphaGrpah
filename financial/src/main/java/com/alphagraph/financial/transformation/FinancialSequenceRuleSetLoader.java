...
/**
 * Loads the 5 named Stage 3 Financial sequence thresholds from
 * {@code common.rule_definitions}/{@code rule_conditions} - same loading shape as
 * {@code market.transformation.MarketSequenceRuleSetLoader}/{@code ownership.transformation.OwnershipSequenceRuleSetLoader}:
 * named scalar constants (single-{@code ALWAYS}-condition rules), never a metric-scoring ladder.
 * Java reads the stored {@code threshold} back directly via {@link #ruleThreshold}, never runs it
 * through {@code ArithmeticRuleEvaluator}.
 */
@Component
class FinancialSequenceRuleSetLoader {

    private static final List<String> RULE_NAMES = List.of(
        "stage3-business-acceleration-max-gap", "stage3-earnings-cycle-max-quarter-gap",
        "stage3-multi-quarter-earnings-min-persistence", "stage3-interest-relief-max-gap",
        "stage3-financial-sequence-max-age"
    );

    private final JdbcTemplate jdbcTemplate;

    FinancialSequenceRuleSetLoader(JdbcTemplate jdbcTemplate) {
