 ...
 */
@Component
class FinancialTransformationSequenceWriter {

    private static final int RULE_VERSION = 1;

    private final JdbcTemplate jdbcTemplate;

    FinancialTransformationSequenceWriter(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    void write(FinancialSequenceResult result) {
        UUID sequenceId = jdbcTemplate.query(
            """
            INSERT INTO financial.transformation_sequences (
        ...
        jdbcTemplate.update("DELETE FROM financial.transformation_sequence_reasons WHERE sequence_id = ?", sequenceId);
        for (ReasonCode reason : result.reasons()) {
            jdbcTemplate.update(
                "INSERT INTO financial.transformation_sequence_reasons (id, sequence_id, reason_code, metric_value, evidence_reference) VALUES (?, ?, ?, ?, ?)",
