...
/**
 * Loads every active {@code decision-swing-*} and {@code decision-longterm-*} rule from
 * common.rule_definitions/rule_conditions into one combined {@link RuleSet} - same pattern as
 * every prior engine's RuleSetLoader (e.g. corporate.signal.CorporateSignalRuleSetLoader), just a
 * broader name prefix since {@link DecisionScoringEngine} partitions the two horizons itself.
 */
@Component
class DecisionRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    DecisionRuleSetLoader(JdbcTemplate jdbcTemplate) {
    ...
    RuleSet loadActiveRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'decision-%'"
        );
