...
/**
 * decision's own raw-SQL reader over MARKET's package-private Stage 1-3 tables - same pattern as
 * {@link FinancialOpportunityReader}. Market's evidence date column is {@code trade_date} (daily
 * cadence, not quarterly - confirmed by direct read).
 */
@Component
class MarketOpportunityReader extends AbstractDomainSequenceReader {

    private final JdbcTemplate jdbcTemplate;

    MarketOpportunityReader(JdbcTemplate jdbcTemplate) {
        super(jdbcTemplate, "market");
    ...
    List<EvidenceObservation> findLatestEvidence(UUID instrumentId, LocalDate asOfDate) {
        return jdbcTemplate.query(
            """
            SELECT DISTINCT ON (metric_name) metric_name, trade_date, value, prior_value, change, confidence, source
            FROM market.transformation_evidence WHERE instrument_id = ? AND trade_date <= ? ORDER BY metric_name, trade_date DESC
            """,
            (rs, rowNum) -> new EvidenceObservation(
                rs.getString("metric_name"), rs.getDate("trade_date").toLocalDate(),
    ...
    Optional<InflectionState> findLatestInflection(UUID instrumentId, LocalDate asOfDate) {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
            ...
            FROM market.inflection_states WHERE instrument_id = ? AND as_of_date <= ? ORDER BY as_of_date DESC LIMIT 1
            """,
            instrumentId, Date.valueOf(asOfDate)
        );
        if (rows.isEmpty()) {
