...
/**
 * decision's own raw-SQL reader over Capital Allocation's package-private Stage 1-3 tables -
 * these live in the {@code corporate} schema/module (package {@code
 * com.alphagraph.corporate.transformation}, confirmed by direct read - there is no dedicated
 * {@code capitalallocation} module). Same pattern as {@link FinancialOpportunityReader}.
 */
@Component
class CapitalAllocationOpportunityReader extends AbstractDomainSequenceReader {

    private final JdbcTemplate jdbcTemplate;

    CapitalAllocationOpportunityReader(JdbcTemplate jdbcTemplate) {
        super(jdbcTemplate, "corporate");
    ...
    List<EvidenceObservation> findLatestEvidence(UUID instrumentId, LocalDate asOfDate) {
        return jdbcTemplate.query(
            ...
            FROM corporate.transformation_evidence WHERE instrument_id = ? AND as_of_date <= ? ORDER BY metric_name, as_of_date DESC
            """,
    ...
    Optional<InflectionState> findLatestInflection(UUID instrumentId, LocalDate asOfDate) {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
            ...
            FROM corporate.inflection_states WHERE instrument_id = ? AND as_of_date <= ? ORDER BY as_of_date DESC LIMIT 1
            """,
            instrumentId, Date.valueOf(asOfDate)
        );
        if (rows.isEmpty()) {
        ...
        List<ReasonNote> reasons = jdbcTemplate.query(
            "SELECT reason_code, metric_value, evidence_reference FROM corporate.inflection_state_reasons WHERE state_id = ?",
