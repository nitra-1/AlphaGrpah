...
/** Loads the active management-* rules from common.rule_definitions/rule_conditions into a {@link RuleSet} - same pattern as every prior engine's RuleSetLoader (Modules 1.5-2.4). */
@Component
class ManagementRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    ManagementRuleSetLoader(JdbcTemplate jdbcTemplate) {
    ...
    RuleSet loadActiveRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'management-%'"
        );
