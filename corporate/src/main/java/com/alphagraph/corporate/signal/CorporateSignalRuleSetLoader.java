...
/** Loads the active corporate-* rules from common.rule_definitions/rule_conditions into a {@link RuleSet} - same pattern as every prior engine's RuleSetLoader. */
@Component
class CorporateSignalRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    CorporateSignalRuleSetLoader(JdbcTemplate jdbcTemplate) {
    ...
    RuleSet loadActiveRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'corporate-%'"
        );
