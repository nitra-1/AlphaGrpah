...
/**
 * Loads the active orderbook-* rules from common.rule_definitions/rule_conditions into a
 * {@link RuleSet} - same pattern as every prior engine's RuleSetLoader (Modules 1.5-1.9), e.g.
 * risk.engine.RiskRuleSetLoader.
 */
@Component
class OrderBookRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    OrderBookRuleSetLoader(JdbcTemplate jdbcTemplate) {
    ...
    RuleSet loadActiveRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'orderbook-%'"
        );
