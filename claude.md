...
Next: Module 1.8+ - the remaining Phase 1 engines (Sector/Risk), both currently empty module scaffolds. Sector Engine likely needs cross-cutting reads (aggregating other engines'/modules' scores by sector) rather than owning fresh source data, and Risk Engine explicitly aggregates Fundamental/Technical/Ownership/Valuation risk per the roadmap - both will probably need intelligence-bridging given they consume other domains' already-computed outputs, not just raw data.
