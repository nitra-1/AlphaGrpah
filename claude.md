...
Next: Module 2.5 - Management Commentary Engine, not yet built. Will follow the same Stage 2 pattern (a new ManagementExtractor implementing DocumentExtractor, narrow prompt covering only revenue/margin guidance, capex, demand, pricing, competition, risk - populating commitmentLevel on its facts, unlike OrderExtractor). Output design agreed but not yet built: two layers mirroring Order Book's shape - Layer 1 immutable per-document Observations (one guidance statement, one document, never revised), Layer 2 a derived Historical Guidance Ledger (trend/persistence/acceleration across quarters) feeding a RuleSet-driven Engine producing Growth Visibility / Guidance Trend / Management Credibility. User explicitly scoped this pattern to Module 2.5 onward, not retroactively applied to Phase 1's already-complete engines (Technical/Fundamental/Institutional/Sector/Risk) - that would be a much larger, separate decision. Real data-sourcing question still open: Module 2.1 currently only collects Exchange Announcements (which do carry some investor presentations and management remarks as PDF attachments already), not dedicated Conference Call transcripts or Press Releases - user's agreed phased plan (2A: build engines against what NSE's existing feed already brings in; 2B: add dedicated IR-site/transcript/press-release collectors later; 2C: external sources) means this is deferred, not blocking.
