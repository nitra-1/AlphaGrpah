...
Next: Module 2.8 - Corporate Signal Engine (combine Order Book/Management Commentary/News Catalyst/Corporate Event signals into one Corporate Score, narrower in scope than the original Phase 1 roadmap's never-built Scoring/Decision Engine which would blend in Technical/Fundamental/Institutional/Sector/Risk too - that remains separate, not-yet-requested future work), then 2.9 (AI Analyst - explicitly "explains, never calculates," reasons only over structured signals and the new knowledge graph, never raw documents) and 2.10 (Dashboard).
