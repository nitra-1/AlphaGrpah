...
Phase 2 (Corporate Intelligence) is now complete: Corporate Event Engine (2.3), Order Book Engine (2.4), Management Commentary Engine (2.5), News Intelligence Engine (2.6), all built on the shared three-stage Document Intelligence pipeline (2.2 revived + retrofit). Still explicitly deferred, not started: retroactively applying the Observation+Historical Ledger two-layer pattern to Phase 1's already-complete engines (Technical/Fundamental/Institutional/Sector/Risk) - out of scope unless the user asks for it as a separate, deliberate decision. Also deferred: Phase 2's 2B/2C document-collector expansion (dedicated Conference Call transcript / Investor Presentation collectors, beyond what Exchange Announcements and the new News RSS feeds already bring in). Next: Phase 3 - Decision Intelligence, per the roadmap.
