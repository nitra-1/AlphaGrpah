 ...
 * day", same convention {@code ownership.transformation.OwnershipTransformationWriter} uses.
 * Reasons are deleted and reinserted every time, matching the parent's upsert semantics.
 */
@Component
class MarketInflectionWriter {

    private static final int RULE_VERSION = 1;

    private final JdbcTemplate jdbcTemplate;

    MarketInflectionWriter(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    void write(MarketInflectionResult result) {
        UUID stateId = jdbcTemplate.query(
            """
            INSERT INTO market.inflection_states (
        ...
        jdbcTemplate.update("DELETE FROM market.inflection_state_reasons WHERE state_id = ?", stateId);
        for (ReasonCode reason : result.reasons()) {
            jdbcTemplate.update(
                "INSERT INTO market.inflection_state_reasons (id, state_id, reason_code, metric_value, evidence_reference) VALUES (?, ?, ?, ?, ?)",
