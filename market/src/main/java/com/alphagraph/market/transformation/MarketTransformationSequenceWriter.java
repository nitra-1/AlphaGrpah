 ...
 */
@Component
class MarketTransformationSequenceWriter {

    private static final int RULE_VERSION = 1;

    private final JdbcTemplate jdbcTemplate;

    MarketTransformationSequenceWriter(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    void write(MarketSequenceResult result) {
        UUID sequenceId = jdbcTemplate.query(
            """
            INSERT INTO market.transformation_sequences (
        ...
        jdbcTemplate.update("DELETE FROM market.transformation_sequence_reasons WHERE sequence_id = ?", sequenceId);
        for (ReasonCode reason : result.reasons()) {
            jdbcTemplate.update(
                "INSERT INTO market.transformation_sequence_reasons (id, sequence_id, reason_code, metric_value, evidence_reference) VALUES (?, ?, ?, ?, ?)",
