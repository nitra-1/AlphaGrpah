...
/**
 * Loads the 4 named Stage 3 Market sequence thresholds from
 * {@code common.rule_definitions}/{@code rule_conditions} - same loading shape every
 * {@code *RuleSetLoader} already uses (e.g. {@code risk.engine.RiskRuleSetLoader}), but these are
 * named scalar constants (single-{@code ALWAYS}-condition rules), never a metric-scoring ladder -
 * see {@code V16__seed_stage3_market_sequence_rules.sql}'s own javadoc for why. Java reads the
 * stored {@code threshold} back directly via {@link #ruleThreshold}, never runs it through
 * {@code ArithmeticRuleEvaluator}.
 */
@Component
class MarketSequenceRuleSetLoader {

    private static final List<String> RULE_NAMES = List.of(
        "stage3-delivery-led-accumulation-max-gap", "stage3-stealth-formation-max-window",
        "stage3-market-recognition-max-gap", "stage3-sequence-max-age"
    );

    private final JdbcTemplate jdbcTemplate;

    MarketSequenceRuleSetLoader(JdbcTemplate jdbcTemplate) {
    ...
    /** Reads a named rule's single ALWAYS condition's threshold directly - not a metric evaluation, just a stored constant lookup. Falls back to {@code defaultValue} if the rule is missing or inactive, same defensive fallback every rule-driven engine in this codebase already has for an unseeded environment. */
