...
/**
 * Loads every active {@code ownership-transformation-*} rule from
 * common.rule_definitions/rule_conditions into one combined {@link RuleSet} - same pattern as
 * {@code ownership.deals.DealMaterialityRuleSetLoader}, just this engine's own name prefix.
 */
@Component
class OwnershipTransformationRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    OwnershipTransformationRuleSetLoader(JdbcTemplate jdbcTemplate) {
    ...
    RuleSet loadActiveRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'ownership-transformation-%'"
        );
