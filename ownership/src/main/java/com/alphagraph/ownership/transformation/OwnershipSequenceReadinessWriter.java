...
@Component
class OwnershipSequenceReadinessWriter {

    private final JdbcTemplate jdbcTemplate;

    OwnershipSequenceReadinessWriter(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    void write(OwnershipSequenceReadinessResult result) {
        jdbcTemplate.update(
            """
            INSERT INTO ownership.transformation_sequence_readiness (id, instrument_id, symbol, as_of_date, history_periods, readiness)
            VALUES (?, ?, ?, ?, ?, ?)
