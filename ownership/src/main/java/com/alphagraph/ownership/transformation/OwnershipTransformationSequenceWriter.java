...
/**
 * Upserts one {@code (instrument_id, as_of_date, sequence_type)} row - a stock may legitimately
 * have multiple simultaneous sequences, so unlike every Stage 2 writer this key includes
 * {@code sequence_type} too. Reasons deleted and reinserted every time, same convention every
 * Stage 2 writer already uses.
 */
@Component
class OwnershipTransformationSequenceWriter {

    private static final int RULE_VERSION = 1;

    private final JdbcTemplate jdbcTemplate;

    OwnershipTransformationSequenceWriter(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    void write(OwnershipSequenceResult result) {
        UUID sequenceId = jdbcTemplate.query(
            """
            INSERT INTO ownership.transformation_sequences (
        ...
        jdbcTemplate.update("DELETE FROM ownership.transformation_sequence_reasons WHERE sequence_id = ?", sequenceId);
        for (ReasonCode reason : result.reasons()) {
            jdbcTemplate.update(
                "INSERT INTO ownership.transformation_sequence_reasons (id, sequence_id, reason_code, metric_value, evidence_reference) VALUES (?, ?, ?, ?, ?)",
