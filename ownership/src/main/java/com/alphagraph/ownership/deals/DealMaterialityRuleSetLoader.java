...
/**
 * Loads every active {@code deal-materiality-*} rule from common.rule_definitions/rule_conditions
 * into one combined {@link RuleSet} - same pattern as every other engine's RuleSetLoader (e.g.
 * {@code decision.engine.DecisionRuleSetLoader}), just this engine's own name prefix.
 */
@Component
class DealMaterialityRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    DealMaterialityRuleSetLoader(JdbcTemplate jdbcTemplate) {
    ...
    RuleSet loadActiveRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'deal-materiality-%'"
        );
