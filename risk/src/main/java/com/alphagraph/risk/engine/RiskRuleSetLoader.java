...
/**
 * Loads the active risk-* rules from common.rule_definitions/rule_conditions into a
 * {@link RuleSet} - same pattern as every prior engine's RuleSetLoader (Modules 1.5-1.8). Named
 * distinctly (not just "RuleSetLoader") for the same reason those did.
 */
@Component
public class RiskRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    public RiskRuleSetLoader(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public RuleSet loadActiveRiskRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'risk-%'"
        );
