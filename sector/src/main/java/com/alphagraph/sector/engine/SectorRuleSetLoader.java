...
/**
 * Loads the active sector-* rules from common.rule_definitions/rule_conditions into a
 * {@link RuleSet} - same pattern as every prior engine's RuleSetLoader (Modules 1.5-1.7). Named
 * distinctly (not just "RuleSetLoader") for the same reason those did - Spring assigns bean names
 * from the simple class name by default, and this exact collision has hit two of the three prior
 * engines already.
 */
@Component
public class SectorRuleSetLoader {

    private final JdbcTemplate jdbcTemplate;

    public SectorRuleSetLoader(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public RuleSet loadActiveSectorRules() {
        List<Map<String, Object>> ruleRows = jdbcTemplate.queryForList(
            ...
            "WHERE active = true AND name LIKE 'sector-%'"
        );
