...
@Component
class SectorSequenceReadinessWriter {

    private final JdbcTemplate jdbcTemplate;

    SectorSequenceReadinessWriter(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    void write(SectorSequenceReadinessResult result) {
        jdbcTemplate.update(
            """
            INSERT INTO sector.transformation_sequence_readiness (id, instrument_id, symbol, as_of_date, history_sessions, readiness)
            VALUES (?, ?, ?, ?, ?, ?)
